  ...
  return (
    <div>
      ...
      <div className="stat-grid">
        <StatCard label="My Assigned Blocks" value={mine.length} icon={<Layers size={12} />} />
        <StatCard label="In Progress" value={inProgress} icon={<Activity size={12} />} />
        <StatCard label="Hours Logged" value={`${hoursLogged}h`} icon={<Clock size={12} />} />
      </div>
      ...
      <div className="card" style={{ marginBottom: 24 }}>
        ...
        <table className="data-table">
          ...
          <tbody>
            {mine.map((b) => {
              ...
              return (
                <tr key={b.id}>
                  ...
                  <td>{b.type}</td>
                  <td><StatusBadge stage={b.status} /></td>
            ...
            {mine.length === 0 && <tr><td colSpan="5" style={{ textAlign: 'center', color: 'var(--text-secondary)', padding: 20 }}>No blocks assigned to you yet.</td></tr>}
          </tbody>
        </table>
      </div>
      ...
      {myRejections.length > 0 && (
        <div className="card">
          ...
          {myRejections.map((a) => {
            ...
            const mgr = mockUsers.find((u) => u.id === a.reviewedBy);
            return (
              <div key={a.id} className="rejection-feedback">
                <div style={{ fontFamily: 'var(--font-display)', fontWeight: 600 }}>{block?.name}</div>
                <div style={{ fontSize: 12, color: 'var(--text-secondary)', marginBottom: 6 }}>
                  Rejected by {mgr?.name} · {relativeTime(a.reviewedAt)}
                </div>
