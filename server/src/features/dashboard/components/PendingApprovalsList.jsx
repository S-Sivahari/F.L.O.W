...
import { _getAllBlocks } from '../../../services/blocks.service.js';
    ...
    <div className="card">
      ...
      {pending.length === 0 ? <EmptyState message="No items awaiting review." /> : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {pending.map((a) => {
            const block = _getAllBlocks().find((b) => b.id === a.blockId);
              ...
              <div key={a.id} style={{ border: '1px solid var(--border)', borderRadius: 6, padding: 12 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 8 }}>
                  <div>
                    ...
                    <div style={{ fontSize: 11, color: 'var(--text-secondary)' }}>{eng?.name} · submitted {relativeTime(a.submittedAt)}</div>
                  </div>
